#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""llm_adapter.py — OpenAI-compatible LLM client for jd-truth-detector."""
...
def resolve_config(env_prefix: str = "JTD", config_path: str | None = None) -> dict:
    result: dict = {}
    # 1. Try env vars
    ...
    paths.append(Path.home() / ".config" / "jd-truth-detector" / "config.yaml")
    for p in paths:
        if p.exists():
            import yaml
            try:
